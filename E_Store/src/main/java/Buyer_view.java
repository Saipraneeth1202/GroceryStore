...
@WebServlet("/Buyer_view")
public class Buyer_view extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String template1 = "SELECT EMAIL, PASSWORD FROM BUYER_DETAILS WHERE EMAIL=?";
		String template2 = "INSERT INTO BUYER_DETAILS VALUES(?, ?, ?, ?)";
		...
		try {
			...
			if (request.getParameter("whattype").equals("REGISTER")) {
				...
				if (rs.next()) {
					out.println(
							"<h1>User already exists. You need to Click on Login. You will be redirected to home page again : )</h1>");
				} else {

					PreparedStatement inserter = con.prepareStatement(template2);
					inserter.setString(1, request.getParameter("name"));
					inserter.setString(2, request.getParameter("email"));
					inserter.setString(3, request.getParameter("password"));
					inserter.setDate(4, Date.valueOf(request.getParameter("dob")));
					...
					session.setAttribute("numpg", "10");
					session.setAttribute("search", "");
					out.print("<META http-equiv=\"refresh\" content=\"2;URL=http://localhost:8080/E_Store/Buyer_home.jsp\"");
				}
			} else {
				PreparedStatement query = con.prepareStatement(template1);
				...
				if (!rs.next()) {
					out.println(
							"<h1>User Not Found. You Need to Register before Login if you are a new User. You Will be Redirected to Home page again: )</h1>");
		...
		out.print("<META http-equiv=\"refresh\" content=\"2;URL=http://localhost:8080/E_Store/Main_Interface.jsp\"");
		out.println("</BODY>\n" + "</HTML>");
	}

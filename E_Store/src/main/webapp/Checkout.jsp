...
<body>
	...
	<div align="center" id="cart_space">
		<h1 align="center">
			<i> -- Items Purchased will be displayed here -- </i>
		</h1>
		<%
		...
		try {
			...
			if (!rs.next()) {
				...
				str = str
				+ "<TH> ITEM_ID </TH><TH> ITEM NAME</TH><TH> CATEGORY </TH><TH> QUANTITY </TH><TH> PRICE</TH><TH> VENDOR_EMAIL </TH><TH> AMOUNT </TH></TR>\n";
				String columnValue;
				int k = 0;
				float f = 0, total = 0;
				while (rs.next()) {
			...
			for (int i = 1; i <= columnsNumber - 1; i++) {
				...
				if (rsmd.getColumnName(i).equals("IMAGE") || rsmd.getColumnName(i).equals("BUYER_EMAIL")) {
					continue;
				}
				if (rsmd.getColumnName(i).equals("QUANTITY")) {
					k = Integer.parseInt(columnValue);
				}
				if (rsmd.getColumnName(i).equals("PRICE")) {
					f = Float.parseFloat(columnValue);
				}
			...
			str = str + "</TR>\n";
				...
				out.println("<form method=\"post\" action=\"Checkout1\">");
				out.println("<button name=\"bt1\" onclick=\"return test3()\" > Pay Now </button>");
				out.println("</form>");
				con.close();
			}
	...
	<script>
		function test3() 
		{
			alert("Amount Paid. Database and Stock Updated Successfully. Cart will be deleted. Your Transaction will be Updated on the Table");
			return true;
		}
	</script>
